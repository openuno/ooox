...
uno.xml.rendercontrol=function(target,configurl,dataurl,_form,_onfinish,mode){
	var pmode='control';if(mode){pmode='control'+mode;}var pidx=uno.xml._fastinstance();uno.xml.processes[pidx]=new uno.xml.process(pmode,target,configurl,dataurl,_form,_onfinish);uno.xml.startprocess(pidx);return false;};
 ...
 if(!_elm){
  ...
  if(!hasfile){
   if(isjson){//JSON POST (simulation)
    ...
    uno.xml._asyncjobs[idx].req.send(jQuery(_elm).serialize());
  }}else{
...
uno.xml._rendercontrol=function(target,template,data,_mode,_outcodes,pidx){if(!data){if(target){return null;}else{return '';}}template=uno.xml._preloadntt(template,false,pidx);
 ...
 for(v=0;v<xconsts.length;v++){s=ooo.xatt(xconsts[v],'tagname');
 consts[v]=uno.xml._varpickup(s,data,ooo.xatt(xconsts[v],'pickup'));
 ...
 consts_name[v]=ooo.xatt(xconsts[v],'substitution');consts[v]=ooo.escape(consts[v],ooo.xatt(xconsts[v],'escape'));
 ...
 for(rt=0;rt<rowtypes.length;rt++){
  ...
  //ROWTYPE FETCHES --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   for(x=0;x<maxrows;x++){v=0;
    //Prepare variables
    for(v=0;v<xvars.length;v++){s=ooo.xatt(xvars[v],'tagname');s=ooo._XTRsubs(s,const_conds,const_conds_name,true);s=ooo._XTRsubs(s,consts,consts_name,true);
    ...
    //Prepare switches
    vins=v+6;var swflag=false;
    for(v=0;v<xswitches.length;v++){swflag=false;nn=xswitches[v].getElementsByTagName('match');s=xswitchesv[v];s=ooo._XTRsubs(s,vars,vars_name);vars[vins+v]='';
	 if(ooo.xatt(xswitches[v],'compare')=='contains'){for(i=0;i<nn.length;i++){if(s.indexOf(ooo.xatt(nn[i],'value'))>-1){
s=ooo.ixml(nn[i],true);s=ooo._XTRsubs(s,conds,conds_name);s=ooo._XTRsubs(s,vars,vars_name);s=ooo._XTRsubs(s,const_conds,const_conds_name);
s=ooo.escape(s,ooo.xatt(xswitches[v],'escape'));vars[vins+v]=s;i=100;swflag=true;}
	 }}else{for(i=0;i<nn.length;i++){if(s==ooo.xatt(nn[i],'value')){
s=ooo.ixml(nn[i],true);s=ooo._XTRsubs(s,conds,conds_name);s=ooo._XTRsubs(s,vars,vars_name);s=ooo._XTRsubs(s,const_conds,const_conds_name);
s=ooo.escape(s,ooo.xatt(xswitches[v],'escape'));vars[vins+v]=s;i=100;swflag=true;}}}
	if(!swflag){for(i=0;i<nn.length;i++){if(ooo.xatt(nn[i],'default')=='true'){s=ooo.ixml(nn[i],true);s=ooo._XTRsubs(s,conds,conds_name);s=ooo._XTRsubs(s,vars,vars_name);s=ooo._XTRsubs(s,const_conds,const_conds_name);s=ooo.escape(s,ooo.xatt(xswitches[v],'escape'));vars[vins+v]=s;}}}}
    for(v=0;v<vars.length;v++){if(vars[v]==null){vars[v]='';}if(vars[v]=='undefined'){vars[v]='';}}
    //Prepare inlines
    for(v=0;v<xins.length;v++){
     s=ooo.xatt(xins[v],'tagname');prel=ooo.xatt(xins[v],'preload');vins=xvars.length+6+v+xswitches.length;
 ...
 if(uno.debug){alert(outcodes.join(','))}
 for(v=0;v<outcodes.length;v++){try{eval(outcodes[v]);}catch(ocex){ooo.err(ocex);}}outcodes=null;outcodes=new Array();}
